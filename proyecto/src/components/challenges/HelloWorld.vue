...
<script>
  export default {
    ...
    methods:{
      ...
      calculateTime(){
        if (this.timeOfDay<=12 && this.timeOfDay>0)
          return this.buenos="Buenos dias"
        else if (this.timeOfDay >=13 && this.timeOfDay<=18)
          return this.buenos="Buenas tardes"
        else if(this.timeOfDay >=19 && this.timeOfDay<=24)
          return this.buenos="Buenas noches"
      },
      nameCreate(){
        return this.name_customer || "No tiene nombre!"
      },
    },
    computed:{
        getMultiplication(){
